...
const result = segments.reduce(
    (previous, current, index, array) => {
        if (current.y1 === current.y2) {
            ...
            // Diagonal line
            while (previous.grid.length <= Math.max(current.y1, current.y2)) {
                previous.grid.push([]);
            }

            const xIncrement = current.x1 < current.x2 ? 1 : -1;
            const yIncrement = current.y1 < current.y2 ? 1 : -1;
            const steps = Math.abs(current.x1 - current.x2);

            for (let ii = 0; ii <= steps; ii++) {
                const x = current.x1 + ii * xIncrement;
                const y = current.y1 + ii * yIncrement;

                while (previous.grid[y].length <= x) {
                    previous.grid[y].push(0);
                }

                previous.grid[y][x]++;
                if (previous.grid[y][x] === 2) {
                    previous.score++;
                }
            }
        }
